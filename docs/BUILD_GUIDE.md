...
Every phase ends with an artifact on a branch. Which gates are HUMAN depends on the project's profile (step 9a). Standard (default): human gates at (a) — merge of the intent PR; (b) — merge of the spec+plan PR; (e) — merge of the build PR (+ a release label only where a real production exists). Gates (c) and (d) are passed by the confidence gate (step 16) and, on failure, PARKED (label sdlc:needs-human, nobody notified). Full: (c) and (d) are also human gates — an approving review + label (sdlc:c-approved, sdlc:d-approved) on the single build PR the article keeps open through Build → Test → Review. Lite: only (a) and (e) are human; (b) is confidence-gated too (withdrawn, decision 21; in the code until step 16a). Gate (f): triage of each incident intent PR — merge (fix now) / label (schedule) / close with a reason (dismiss). At every human gate, 'ask for changes' = review comments on that PR, consumed by one fix command (step 8). Nothing ever pages you: finished or parked work waits in the review queue (step 27a).
...
**Implementation notes.** Decided (decision 2) and propagated everywhere: plan.md is produced at the end of phase (b) and approved with spec.md at gate (b) — one gate, at the cheapest point to change course (p.17). The article's enforcement property ('cannot edit files until the engineer accepts the plan') is rebuilt for headless runs as a two-run rule: the planning run has read-only tools (Read/Grep/Glob) and commits plan.md; the implementation run (phase c) has edit/shell tools and starts only after gate (b). The interrogation questions (p.16 step 3) become a fixed prompt whose answers land in plan.md's Risks and an added 'Options not taken' section; plan.md keeps the article's four sections. In phase (c) the pre-commit check of step 15 enforces 'plan.md updated in the same commit'. The Lite profile (step 9a; withdrawn, decision 21; in the code until step 16a) replaces your approval at (b) with the confidence gate; for Standard and Full, plan.md stays at gate (b).
...
**Implementation notes.** Decided (decision 13). Sequence: (e) runs review passes and the fix loop until green, drafts release notes, prepares the release → stops. Approve = merge; only when the project's adapter (9b) declares a real production does a release label become a second act (decision 13): the merge-triggered release workflow (`sdlc-release.yml`, B4) checks the approval itself before running the adapter's `deploy.command`, because a hook fires only inside a Claude session, and the production-gate hook (29) reads the same approval when Claude itself invokes a deploy. The release workflow deploys; the post-deploy band of step 37 is the safety net, with the rehearsed rollback (31) as its action. For a project without a production environment, merge is the whole gate. This is the review you do 'once the work is done': the PR opens with the ≤5-bullet summary of step 27a, and in the Standard profile it is the first time you look at the change since approving spec+plan. [ADAPTED]


#### Step 16a — Deferred review inside a phase: a review panel (reviewer · devil's advocate · conciliator) stands in for you on judgment items, a decisions ledger records every call, and you review once at the phase's end

- **Source tag:** ADAPTED  
- **Phase it belongs to:** b. design (first) · c. build · d. test · e. deploy  
- **Importance:** 4 · High — Decision 21 (2026-09-23). The live runs showed the cost of parking on every judgment item: 6–9 open concerns per first spec, each round a commit, a workflow run and a review, until no single change is read carefully. Deferring those calls to a panel and reviewing once at the phase's end is what 'my input at the end of each step' meant, and it is the article's own p.42 mechanism applied inside a phase. It replaces the Lite profile, which had removed the human from gate (b) against p.14–15.


**Concept from the article.** The article's end state: "Stage 6: Maintenance runs headless, with an independent confidence gate between stages, a deterministic check or an adversarial reviewing agent, deciding whether the previous stage's output continues or is escalated to a human." Where human attention goes: "Human attention concentrates at the gates, reviewing what the agent flagged rather than starting each stage from scratch." The gate that stays human: "A human team mate always makes this call, and accepting the spec is what starts the plan mode play in Stage 3: Build."; 'Nothing is implemented without an accepted plan.' Fresh context as the reviewer's independence: the verifier runs 'a fresh context window once the session believes the work is done. This way the verdict is not colored by the assumptions that produced the code'. Accountability: "Humans remain accountable for every decision that requires judgment."


**Where in the article.**

- p.42 — Stage 6 Maintain › 'Maintenance and closing the loop' – confidence gate, escalation
- p.8 — Plays › 'Human attention concentrates at the gates…'
- p.14 — Stage 2 Design › step 6 – a human accepts the spec
- p.15 — Stage 3 Build › tagline – 'Nothing is implemented without an accepted plan'
- p.27 — Stage 4 Test › feedback loop vs verifier subagent – the verifier's fresh context
- p.6 — Intro › human accountability

**Implementation notes.** Not in the article as a play; assembled from its confidence gate (p.42), its fresh-context reviewers (p.27, p.42) and its accountability rule (p.6). Setting: sdlc.yaml review: deferred (per-change override in status.yaml), off by default. Trigger: inside a phase run, wherever the gate would park on an item of the FIXED list — an open flagged concern in spec.md, contradicting policy skills, an escalate verdict of the adversarial reviewer (its routine / non-routine classification never parks; it only lowers the iteration cap), an Important review finding whose fix the run cannot determine, a verifier disagreement — the run calls the panel instead. Panel: (1) reviewer = the REVIEW.md pass prompt over the item and its context; (2) devil's advocate = the adversarial reviewer, blind to (1); both fresh contexts, and the devil's advocate on a different model than the reviewer (--model on its claude -p run, or the agent's model frontmatter when it runs as a sub-agent; NOTES §11b, §11c), because same-model judges share blind spots; (3) conciliator = a fresh context that reads both verdicts and writes one decision with a rationale of at most five lines. The run applies the decision (closes the concern with the conciliator's text marked 'decided-by: panel', picks the policy reading, chooses the fix) and continues. Ledger: one line per decision in changes/<id>-<slug>/evidence/decisions-<phase>.md (item, the two verdicts in one clause each, the decision, the rationale, the cost); the PR summary at the next human gate opens with 'Decisions taken for you (N)' above the five bullets, and a review comment on any line overturns it through /sdlc-fix (decision 22). Never to the panel, still a park: a risk-list hit, a guardrail file change, a run limit, an infrastructure failure, any merge to main, any deploy. Bounds: each panel call is three claude -p runs charged to the phase's budget (gate.max_budget_usd) and counted against gate.max_panel_calls (its own count since 0.2.15, not a fix iteration); the panel may not raise a limit or edit a guardrail file. Gate checks (step 16) gain: 'every panel decision has a ledger line' and 'no panel decision touched a never-to-panel item'. Build order: B4, after the fix trigger of decision 22 (the overturn route must exist first); removes the Lite profile from state/conventions.py, state/cli.py, the init, design, build and test commands, run_phase.py, template/sdlc.yaml and the build workflow, and retires the Lite tests, when it ships. [ADAPTED]
...
**Implementation notes.** Decided (decision 17): build later, if ever — only when review keeps up. If built: /sdlc-build reads plan.md's 'Files that change' and spawns one worktree per proven file-disjoint task; at most two or three; on any merge conflict, escalate through the gate (16) rather than resolve autonomously. Every worktree inherits the repo's hooks and permissions. Built (session 5, plugin 0.2.19): not built, recorded (decision 17): one person reviews only at phase ends, so a second stream waits on the same review; the framework's runs are unattended and serialised per change by the workflows' concurrency groups, and a parallel worktree would need the conflict escalation designed here first. Revisit when review keeps up.
...
**Implementation notes.** Store the exported mock in changes/<id>/mock/ so (c) and (d) compare screenshots against it. The article does not say how the export relates to spec.md or in what format. Built (session 5, plugin 0.2.19): the convention only — `changes/<id>-<slug>/mock/` holds the exported mock (changes/README.md), the build prompt reads it before the first screen and names the screenshot in plan.md Proof, the test prompt compares the screenshot with it and reports a mock the folder does not hold as a mismatch.
...
**Implementation notes.** Decided (decision 17): create the empty per-project evals/ on day one, write the suite once ~20 real changes exist. Build-order deviation, recorded 2026-09-23: the p.8 graph draws Evals → PR review as a solid prerequisite, and this guide builds the review pass (26, B3) before the eval suites (35, B5); deliberate, because the suites are fed by incidents and live runs that do not exist before B3, while the review pass runs without them. Two suites: (1) per project — /sdlc-init creates an empty evals/ with the p.30 workflow; phase (f) adds one eval per incident (37) and per fixed vulnerability class (38); (2) framework-level — in the plugin repo, triggered by PRs to the plugin. The article's own trigger is the product repo's CLAUDE.md and .claude/**, so a plugin-only suite would miss project-level changes. Do not wire it as a phase (d) job. evals/check.sh and the eval JSON are not shown in the article. Budget depends on the substrate decision (3). Built (session 5, plugin 0.2.19): the framework's own runner, `plugin/evals/run.py` (a copy of the checkout per case, the project's CLAUDE.md, hooks and settings in force — never `--bare` —, `claude -p … --output-format json`, the checks of `checks.yaml` applied in Python: a command's exit code, a file's content or that it is unchanged, the agent's output, a denied tool; a JSON report and a pass-rate exit code), `evals/check.py` in the template (the p.30 `check.sh`), `evals/cases/<id>-<slug>/{prompt.md,checks.yaml}` as the case shape, `plugin/evals/case.py new` for the incident's case (36), the per-project `sdlc-evals.yml` (PRs to CLAUDE.md, .claude/**, evals/**, nightly) and this repository's `framework-evals.yml` (PRs to plugin/**, nightly) over a fixture project initialised with the plugin; the first four cases from the live checks (a protected path denied, the intent skill's shape, a secret kept out, verify before done). `claude plugin eval` (2.1.269+) was read and not used: it loads only the plugin in a throwaway workspace, has no custom-code graders and can be switched off server-side (NOTES §13).
...
**Implementation notes.** Template ships lessons/ (one file per incident, in the intent's change folder or a top-level lessons/ directory); the diagnose agent of step 37 must read it before diagnosing and append the post-mortem when a fix ships. Reviewed like any file — it rides in the fix PR. Built (session 5, plugin 0.2.19): `template/lessons/README.md` (one file per incident, `lessons/<yyyy-mm>-<slug>.md`, the p.49 name; the shape: what happened, root cause, fix, prevention), read first by `/sdlc-maintain` step 1, written by `/sdlc-deploy` step 0b when the fix of an incident change ships, with the eval case of 35 beside it, both listed in plan.md and reviewed in the fix PR.
...
**Implementation notes.** Template ships bands.yaml (copy p.44), a detection-script skeleton with unit tests in the cross-platform runtime, and a scheduled workflow. Decided (decision 15): the first metric is always CI test failure rate (the article's own example; exists on day one, reversible 3σ actions); the project's maintain adapter (9b) adds its own metric once it runs live. Detection stays model-free. 2σ reuses the diagnose agent read-only; 3σ may only open a PR (through gate (e)) or run a runbook you approved in advance — the article says 'act' in the text and 'propose' in the config; treat them as one. Decided (decision 14, amended by decision 26 on 2026-09-23: a rehearsed rollback on a declared production is pre-approved, everything else that touches a running system still waits for 'Go' as a parked item) — split by blast radius, written per route in bands.yaml: CI-scoped, reversible actions (quarantine a flaky test, open a revert PR) are pre-approved and run with no human in the path (p.43–44); anything touching a running system (rollback, config change) waits for your 'Go' (the p.49–50 model) — as a parked item in the queue, not a page. Output intent.md re-enters phase (a) via the triage queue (39). Phase (f) has no 'conclusion': its unit of work is one finding. Runtime: a CI step or an Agent SDK service for webhooks (p.43–44). [ADAPTED: the p.45 example has the agent trigger the rollback pipeline itself at 3σ; decisions 14 and 26 split the routes by blast radius and pre-approve only a rehearsed rollback on a declared production.] Built (session 5, plugin 0.2.19): `plugin/detect/` — `stats.py` (rolling mean and sample standard deviation of the window minus its last eight points, the four Western Electric rules over those eight, `we1` → 3σ, `we2` and `we4` → 2σ, `we3` → 1σ, a flat baseline followed by one bad day is a 3σ breach as p.45 says, optional `direction` and `min_sigma`), `source.py` (the GitHub Actions runs API through `urllib`, one observation per UTC day, a file cache), `bands.py`, `finding.py` (the signature `detect|metric|rule`, `detection.json`), `routes.py` (decisions 14 and 26 resolved per route), `runbooks.py` (`quarantine-flaky-test` adds `--deselect` to pytest's addopts on a branch and opens the PR, never edits the test; `revert-pr`; a project runbook's `command`), `cli.py run | routes | dispatch | go | dismiss`; the scheduled `sdlc-detect.yml` (the runtime: a CI step, not an Agent SDK service — no webhook source exists and decision 7 forbids a runtime dependency) and `sdlc-runbook.yml` (the owner's `sdlc:go`); `/sdlc-maintain`; gate (f)'s `detection` and `route` checks. One finding per metric at a time; a `go` route parks with "Go requested"; nothing notifies.
...
**Implementation notes.** Decided (decision 16): unless your plan includes Claude Security, implement the pattern with a weekly `claude -p` run of a security-review skill in read-only mode, routing findings exactly as the article's steps 5–7 (p.47) say [ADAPTED — the article gives no non-Enterprise alternative]. Keep deterministic scanners (dependency audit, SAST) in CI regardless. Dismissals record a reason and suppress that finding's signature. Built (session 5, plugin 0.2.19): `plugin/scan/` — the weekly read-only review (`sdlc-scan.yml`, one `claude -p` with the `security-baseline` skill over the whole tree, findings in the review pass's JSON shape plus `bounded` and `class`), the routing (`scan/route.py`: at most three Important findings per run become incident intent changes at gate (f) — a bounded one `change_type: fix` with `evidence/suggested-patch.diff`, a wide one `feature`; no route skips gates (a) and (b), which decisions 18 and 21 keep human), the dismissal store shared with 37 (`changes/.dismissed.json`, a scan dismissal never expires), the deterministic scanners per language in the same workflow (`pip-audit`, `bandit`; `npm audit`), reported, never a gate; the vulnerability-class eval is the incident's eval of 36.
...
**Implementation notes.** The queue is the set of open intent PRs labelled 'incident'; fix now = merge (which is also gate (a) for that intent), schedule = label, dismiss = close with a reason. Dismissals suppress that finding's signature; changes to the bands themselves go through a bands.yaml PR so the detection script stays 'version controlled and unit tested' (p.43) — the article says dismissals tune the bands, not that tuning is automatic. Added 2026-09-23 (decision 25): per-finding triage is the whole of gate (f) — there is no phase-level sign-off — and closing a PR at gates (a)–(e) without merging abandons the change; closing an incident intent PR both dismisses the finding and abandons its change. Built (session 5, plugin 0.2.19): the labels `incident`, `schedule`, `sdlc:go` (`conventions.all_labels`), gate (f) as the owner's triage of the incident PR (merge = gate (a) of that change; `schedule` keeps it open; close with a comment = dismiss: `sdlc-abandon.yml` runs `detect/cli.py dismiss --open-pr`, which appends the signature and the reason to `changes/.dismissed.json` on `sdlc/<id>/dismiss` and opens the dismissal PR against the default branch — no comment, no dismissal); a bands change is a `bands.yaml` PR, the detection run never writes it.
...
**Implementation notes.** Now: the generated PR descriptions (27a) carry two counters — first-pass merge (yes/no) and fix iterations — which give the article's first-pass share and rework cycles for free. Later: a scheduled agent run walks changes/*/ and the PR history and opens a short report PR you merge or ignore. Skip the team-only indicators (new-member onboarding time, concurrent sessions per engineer). Built (session 5, plugin 0.2.19; 42.2): `plugin/pr/counters.py` over `changes/*/status.yaml` and `changes/.dismissed.json` — first-pass merge share, fix iterations per shipped change, panel calls, abandoned, incidents filed / shipped / dismissed, dismissals in force — rendered as the last section of the daily digest issue (`pr/digest.py --root .`): no report PR, no model, pull not push.
