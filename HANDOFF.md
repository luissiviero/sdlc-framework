...
2. The owner's live checks from `docs/PROGRESS.md` ("Live checks the owner can do now"). Read their status from the owner's first message or from the PR #1 checklist; do not assume. If any is unreported, ask once and continue with the rest of B2, marking the dependent step partial.
   ...
   - Hook denial in a live session: **passed on 2026-09-21** in a cloud session with the setup script of `docs/NOTES.md` §3a (tool result `PreToolUse:Edit hook error: Protected path: …`, file unchanged, `Registered 5 hooks from 2 plugins` in the debug log). Caveat recorded in `docs/PROGRESS.md`: an earlier Edit attempt in that same session was caught by Claude Code's settings validator instead of the hook, which means the hook did not fire on that call and the difference is unexplained. Before enabling auto-accept (step 18), repeat the schema-valid edit test in a **fresh** session and treat any non-hook result as a blocker to diagnose.
   - Intent-skill trigger: **passed on 2026-09-20** in a cloud session (the skill loaded, id 0001 was allocated through the CLI, PR #3 on `sdlc-sample-python`). Use the same description style for the policy skills of step 20.
