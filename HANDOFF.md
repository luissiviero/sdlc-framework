...
2. The owner's live checks from `docs/PROGRESS.md` ("Live checks the owner can do now"). Read their status from the owner's first message or from the PR #1 checklist; do not assume. If any is unreported, ask once and continue with the rest of B2, marking the dependent step partial.
   ...
   - Hook denial in a live session: **passed on 2026-09-21** in a cloud session with the setup script of `docs/NOTES.md` §3a (tool result `PreToolUse:Edit hook error: Protected path: …`, file unchanged, `Registered 5 hooks from 2 plugins` in the debug log). Confirmed again the same day in a fresh session whose first tool calls were the Read and the Edit: the hook denied it. An earlier anomaly (one Edit caught by the settings validator instead) did not reproduce; it is recorded in `docs/PROGRESS.md` for the record. Auto-accept (step 18) may be built on the assumption that the hooks fire.
